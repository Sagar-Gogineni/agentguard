...
class DisclosureMethod(str, Enum):
    ...
    METADATA = "metadata"  # Add to response metadata
    HEADER = "header"  # HTTP header (for API deployments)
...
class AgentGuardConfig(BaseModel):
    ...
    system_name: str = Field(
        ..., description="Name of the AI system being deployed"
    )
    provider_name: str = Field(
        ..., description="Legal name of the organization deploying the AI"
    )
    version: str = Field(
        default="0.1.0", description="Version of the AI system"
    )
    ...
    disclosure_method: DisclosureMethod = Field(
        default=DisclosureMethod.PREPEND
    )
    disclosure_text: str = Field(
        default="🤖 This response was generated by an AI system ({system_name}) "
    ...
    log_inputs: bool = Field(
        default=True, description="Log user inputs (ensure GDPR compliance)"
    )
    log_outputs: bool = Field(
        default=True, description="Log AI outputs"
    )
    ...
    human_escalation: HumanEscalation = Field(
        default=HumanEscalation.LOW_CONFIDENCE
    )
